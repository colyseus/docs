...
import { DevicesIcon, GearIcon } from '@primer/octicons-react';
...
<Tabs items={["Client", "Backend config"]}>
    <Tabs.Tab>
    ```js
    try {
...
Use the `auth.oauth.addProvider(providerId, config)` method to add an OAuth provider.
...
```ts filename="Signature"
auth.settings.onParseToken(jwt: string) => Promise<any>
```
...
<Tabs items={["When registering with Email/Password"]}>
    <Tabs.Tab>
    ```ts filename="src/config/auth.ts"
    import { auth } from "@colyseus/auth";
