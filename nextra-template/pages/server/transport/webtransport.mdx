...
WebTransport is a new transport protocol that allows for low-latency, bidirectional, and multiplexed communication between a client and a server. It is built on top of HTTP/3 and QUIC - QUIC is a new transport protocol that is built on top of UDP, which is designed to be more efficient than TCP.
