...
No connection with the server is established yet by creating a `Client` instance. Communication with the server only happens when you join a room.
...
<Callout type="info">
    **Use `Room#onMessage()` from the server-side to receive the messages** - Check out [Server-side API &raquo; Room - onMessage()](/server/room/#onmessage-type-callback) section.
</Callout>
...
<Callout type="info">
    Check out the [State Synchronization » Client-side Callbacks](/state/schema/#client-side) section for more details.
</Callout>
