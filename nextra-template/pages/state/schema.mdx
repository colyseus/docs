...
The state is set on the Room instance, and it is automatically synchronized with all clients in the room.
...
<Callout type="warning">
    **`SetSchema` is only available for JavaScript SDK** - Haxe, C# and Lua SDKs are not supported yet.
</Callout>
...
<Callout type="warning">
    **`CollectionSchema` is only implemented in JavaScript** - Haxe, C#, Lua and C++ clients are not supported yet.
</Callout>
