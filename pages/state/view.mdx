...
The `.view()` field modifier is used to tag a field as only visible to `StateView` instances that contain that `Schema` instance.
...
// ...
    onJoin(client, options) {
        const player = new Player({ name: "Player 1", health: 100, position: 0 });
        this.state.players.set(client.sessionId, player);
