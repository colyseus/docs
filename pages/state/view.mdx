...
`view.add(instance)` is a **one-shot**: it grants visibility of that instance, and nothing else. For a collection that keeps gaining elements, that means calling `.add()` again for every new one — and forgetting a call silently leaves that client behind.
...
Subscribing brings in the collection's current contents as well as future ones, so it replaces the initial `view.add(collection)` rather than supplementing it. It is idempotent — subscribing to an already-subscribed collection is a no-op.
