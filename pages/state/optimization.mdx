...
<Callout type="info">
    The wire format did not change in `@colyseus/schema` 5.0. The encoder became roughly 2.4× faster and allocates about half the heap, but the bytes are identical to 4.x. What 5.0 added is *control*: [delivery modifiers](/state/schema#delivery-modifiers), [quantized floats](/state/schema#quantized-floats-tquantized--tangle), and [streaming collections](/state/streaming).
</Callout>
...
| Unit | Cost | Notes |
|------|------|-------|
| Each instance that changed this tick | 1 byte, plus its `refId` | The `refId` is 1 byte below 128, then 2, then 3 |
| Each changed `Schema` field | **1 byte**, plus the value | The operation and the field index share one byte |
| Each changed collection entry | 1 byte, plus an index of 1 to 3 bytes, plus the value | No packing: the index is separate |
| A `MapSchema` insertion | The above, plus the UTF-8 key | Only on insert. Later updates to that entry omit the key |

A `Schema` field is the cheapest slot Colyseus has. One byte carries both "what happened" and "which field", for every field from the first to the sixty-fourth.
...
**Declare filtered and delivery-modified fields early.** Lookups for `.view()`, `.patchOnly()`, `.fullStateOnly()` and `.stream()` resolve through a bitmask for the first 32 fields. Fields at index 32 and above fall back to a slower metadata lookup on every mutation. This affects server CPU, not bandwidth.
...
Everything above shrinks a patch. These four shrink how much of it each client receives, or how often:

- [**Delivery modifiers**](/state/schema#delivery-modifiers). `.fullStateOnly()` for room configuration set once in `onCreate()`, so it never touches a tick patch. `.patchOnly()` for data a late joiner does not need. `.noSync()` for fields that should never leave the server.
...
**Fields left `undefined`, and fields set to `null`, are skipped entirely in the full state sync.** An `.optional()` field that is never assigned costs nothing. So does a [`.deprecated()`](/state/schema#versioning-and-backwardsforwards-compatibility) field, which has no setter and can never hold a value.
