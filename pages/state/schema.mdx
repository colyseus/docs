...
<Callout type="warning">
    **Some types are only available for JavaScript SDK:** `cstring`, `bigUint64`, `bigInt64`, `varUint`, `varInt`, `varBigUint`, `varBigInt`, `varFloat32`, `varFloat64`

    **`cstring` type requires the platform to supply a TextEncoder (and TextDecoder) implementation.** You may supply a polyfill yourself.
</Callout>
...
- `cstring`: utf8 null-terminated string type.
- `bigUint64`, `bigInt64`: unsigned / signed 64-bit bigint type.
- `varUint`, `varInt`: unsigned / signed variable-length integer type. (encoded with [LEB128](https://en.wikipedia.org/wiki/LEB128))
- `varBigUint`, `varBigInt`: unsigned / signed variable-length bigint type. (encoded with [LEB128](https://en.wikipedia.org/wiki/LEB128))
- `varFloat32`, `varFloat64`: variable-length floating-point number types. (encoded with [LEB128](https://en.wikipedia.org/wiki/LEB128))
...
| `"cstring"` | null-terminated utf8 strings | string must not contain a null-byte ("\x00") |
