...
<Callout type="info">
    **A range symmetric about zero carries an exact zero.** When `min` is exactly `-max`, as in `t.quantized({ min: -1, max: 1 })` for an input axis or `{ min: -150, max: 150 }` for a velocity, the range is laid out so that `min`, `0` and `max` all land exactly on a step. A released axis reads back as `0`, so a `=== 0` test fires, and a velocity you set to zero comes to rest instead of creeping. Evening out the grid costs one of the `2^bits` codes, so the step is a hair coarser and nothing else changes.
    ...
    This needs `@colyseus/schema` 5.0.27 on the server and a client SDK released alongside it: Unity 0.18.4, Haxe 0.18.2, Defold 0.18.0, or the native C SDK 0.18.3. The Godot, GameMaker, Flutter and Swift bindings wrap that core and carry it from their next release. Before these versions a symmetric range placed zero half a step off the grid and rounding lifted it to the next step, which showed up as a stuck fraction of throttle on a released stick, or a resting body drifting forever.
</Callout>
