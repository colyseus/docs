...
| `.unreliable()` | Tick patches carry the field on the unreliable channel — see [Delivery modifiers](#delivery-modifiers) |
| `.patchOnly()` | Sent on tick patches only, never in a full state sync — see [Delivery modifiers](#delivery-modifiers) |
| `.fullStateOnly()` | Sent in the full state sync only, never in a tick patch — see [Delivery modifiers](#delivery-modifiers) |
| `.stream()` | Drip-feed a collection's additions across ticks — see [Streaming](/state/streaming) |
...
<Callout type="info">
    `.unreliable()` and `.patchOnly()` also exist as decorators (`@unreliable`, `@patchOnly`). `.fullStateOnly()`, `.stream()` and `.priority()` are builder-only.
</Callout>
