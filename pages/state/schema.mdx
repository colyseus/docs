...
<Callout type="info">
    All three are server-side encoder concerns. Nothing changes on the client: a decoder just receives the field, or doesn't. No client-side setup is required.
</Callout>
...
<Callout type="warning">
    Requires a datagram-capable transport (WebTransport). Over plain WebSocket every field is delivered reliably and the modifier has no effect — it is safe to leave in your schema either way.
</Callout>
