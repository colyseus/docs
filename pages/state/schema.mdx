...
<Tabs items={["TypeScript", "JavaScript", "JavaScript (legacy)"]} storageKey="ts-or-js">
    <Tabs.Tab>
    ...
    <Tabs.Tab>
        <Callout type="info">
            The `schema()` helper is the recommended way to define state structures in plain JavaScript. Since schema 5 (Colyseus 0.18) each field takes a `t.*` builder — raw strings like `"string"` are no longer accepted and throw at definition time.
        </Callout>
    ...
    <Tabs.Tab>
        <Callout type="error">
            The `defineTypes()` method was **removed** in Colyseus 0.18 (schema 5) — this legacy example no longer runs and is kept only to help you recognize old code. Migrate to `schema()` with `t.*` builders.
        </Callout>
...
<Tabs items={["TypeScript", "JavaScript"]} storageKey="ts-or-js">
    ...
    const Item = schema({/* base Item fields */});
