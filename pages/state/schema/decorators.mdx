...
description: "Define Colyseus schema state with TypeScript @type() decorators: tsconfig setup, collections, default values, inheritance, @view(), @deprecated() and delivery modifiers. Fully supported alternative to the schema() builder."
---
...
The semantics, use cases and cautions are documented in the [main reference → Delivery modifiers](/state/schema#delivery-modifiers). That includes the transport requirement for `@unreliable`, which is experimental and reaches clients only over [WebTransport](/server/transport/webtransport).
...
See [Versioning](/state/schema#versioning-and-backwardsforwards-compatibility) for the full backwards/forwards compatibility workflow.
