...
<Tabs items={["TypeScript", "C#", "Lua", "Haxe"]}>
    ...
    local $ = colyseus.get_state_callbacks(room)
    ```
    </Tabs.Tab>
...
<Tabs items={["TypeScript", "C#", "Lua", "Haxe"]}>
    <Tabs.Tab>
    ```ts {3} filename="client.ts"
    $(room.state).entities.onAdd((entity, sessionId) => {
        // ...
        $(entity).onChange(() => {
            // something changed on `entity`
        })
    });
    ```
    </Tabs.Tab>
...
<Tabs items={["TypeScript", "C#", "Lua", "Haxe"]}>
    ...
    <Tabs.Tab>
    ```cs filename="client.cs"
    callbacks.OnAdd(room.State.players, (string key, Player player) =>
    {
