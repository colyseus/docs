...
description: "React to state changes on the client: register callbacks on Schema instances for property changes, additions, and removals during decoding."
---
...
Registering before the server has sent the collection is fine. A collection the client still holds as `null`, or as the empty instance a generated C# class starts with, counts as not yet received. The callback waits and fires when the first sync, or a later patch, delivers it, replaying the items that arrived with it. A collection a [`StateView`](/state/view) adds later arrives the same way.
...
<Callout type="warning">
    **Not required when using [JavaScript SDK](/getting-started/typescript) or [Defold SDK](/getting-started/defold).** The following section is only required when using statically typed languages in your frontend, such as C#, Haxe, etc.
</Callout>
