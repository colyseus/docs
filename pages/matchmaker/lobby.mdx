---
title: "Lobby Room"
description: "The built-in LobbyRoom provides a live listing of available rooms — the backend for a room browser UI, with automatic updates and filtering."
---
...
- Filter support by room `name`, optionally narrowed by `metadata`.

Rooms flagged [`private` or `unlisted`](/matchmaker/visibility#matchmaking-properties) never appear in the lobby listing. (`locked` rooms remain listed, marked as locked.)
...
Filter the listing by providing the `filter` option when joining the lobby. A filter **must include `name`** — the name check runs unconditionally, so a metadata-only filter matches nothing. Use `metadata` to narrow within a room name:
