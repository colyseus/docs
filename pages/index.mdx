...
- [**<PeopleIcon /> Rooms & Matchmaking**](/room) — From a single Room definition, clients are matched into multiple room instances — with [visibility flags, lobbies, and ranked queues](/matchmaker).
- [**<SyncIcon /> State Synchronization**](/state) — Define your state on the server; clients receive binary delta patches automatically.
- [**<ZapIcon /> Prediction-Ready Netcode**](/netcode) — Built-in client prediction, rollback, interpolation, and lag compensation — new in 0.18.
- [**<CodeIcon /> Full-Stack Type Safety**](/sdk) — Share your server's types with the SDK: typed messages, typed join options, typed state.
- [**<PasskeyFillIcon /> Batteries Included**](/auth) — Authentication (JWT + OAuth), [database services](/database), [room plugins](/room/plugins), [admin panel](/admin), and [dev tools](/tools) out of the box.
- [**<DevicesIcon /> Any Engine**](/getting-started) — Official SDKs for the Web, Unity, Godot, Defold, GameMaker, Construct, Haxe, and more.
- [**<ServerIcon /> Scale & Deploy Anywhere**](/scalability) — Horizontal scaling across processes; [self-host](/deployment) for free or use [Colyseus Cloud](https://cloud.colyseus.io/).
