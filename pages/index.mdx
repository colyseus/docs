...
Ship multiplayer games faster. Colyseus is an open-source Node.js framework for building authoritative game servers - with real-time state synchronization, matchmaking, and effortless integration into any game engine or frontend.

-  [**<PeopleIcon /> Match-making into Rooms**](/room) – From a single Room definition, clients are matched into multiple Room instances.
- [**<SyncIcon /> State Synchronization**](/state) – Automatically synchronize the state from the server with connected clients.
- [**<ServerIcon /> Scalable**](/scalability) – Built for horizontal and/or vertical scalability.
...
<Tabs items={["TypeScript", "JavaScript", "Unity (C#)", "Defold (Lua)", "Haxe"]} storageKey='sdk-tab-language'>
    ...
    client.joinOrCreate('my_room', {/* custom join options */}).then(room => {
