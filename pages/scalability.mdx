---
title: "Scalability"
description: "Scale Colyseus across multiple processes and machines with Redis presence, a shared matchmaking driver, and a load balancer, or let Colyseus Cloud handle it."
---
...
- Each process **has a maximum amount of players** it can handle. _(The exact amount depends on many factors. [See our FAQ](/faq/#how-many-ccu-can-a-colyseus-server-handle).)_
...
<details>
    <summary> <FileMediaIcon/> Check out the diagram of how matchmaking works with scaling infrastructure and multiple processes.</summary>

    ---

    ![Scalability diagram](/diagrams/scalability.webp)
</details>
...
module.exports = {
    apps: [{
        port        : 2567,
        name        : "colyseus",
        script      : "build/index.js", // your compiled entrypoint file
        time        : true,
        watch       : false,
...
Colyseus automatically adds PM2's `NODE_APP_INSTANCE` to the port number. If you specified `2567` as port number, the first instance will listen on port `2567`, the second instance on port `2568`, and so on.

---

Now you're ready to start multiple Colyseus processes.
...
<Callout type="info">
    Prefer Traefik? The [Traefik Load Balancer](/scalability/traefik) guide covers the same role with automatic service discovery and Let's Encrypt certificates.
</Callout>
...
- [Deployment](/deployment) - The production checklist: PM2, HTTPS, health checks
- [Graceful Shutdown](/server/graceful-shutdown) - How deploys drain rooms and notify clients
- [FAQ: How many CCU can a Colyseus server handle?](/faq#how-many-ccu-can-a-colyseus-server-handle) - Capacity planning per process
