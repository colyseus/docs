...
{/* Em-dash headings renamed (punctuation ban); keep old deep links working. */}
<MovedAnchors map={{
    "client--the-render-time-stamp": "/netcode/lag-compensation#client-the-render-time-stamp",
    "server--rewind-targets-to-where-the-shooter-saw-them": "/netcode/lag-compensation#server-rewind-targets-to-where-the-shooter-saw-them",
}} />
...
<SDKTabs>
    ...
    predict.AttachAll("players", new AttachConfig {
        ...
        ["y"] = new PredictFieldOptions { Mode = PredictMode.Lerp, Delay = 100 },
    });
    var input = room.Input(cmd);   // stamp auto-derived from the lerp delay
    ```
    </Tabs.Tab>
    ...
    local input = room:input()   -- stamp auto-derived from the lerp delay
    ```
    </Tabs.Tab>
    ...
    var input = room.input({ type: MoveInput });   // stamp auto-derived from the lerp delay
    ```
    </Tabs.Tab>
    ...
    var input = room.input()   # stamp auto-derived from the lerp delay
    ```
    </Tabs.Tab>
    ...
    predict.attachAll('players', config: {
      'x': FieldOptions(mode: PredictMode.lerp, delay: 100),
      'y': FieldOptions(mode: PredictMode.lerp, delay: 100),
    }, exceptKey: room.sessionId);
    final input = room.input()!;   // stamp auto-derived from the lerp delay
    ```
    </Tabs.Tab>
