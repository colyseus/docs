...
Each input carries a timestamp telling the server *when* the client's view of the world was. This is automatic: once the room enables rewind, the SDK stamps every input, and wiring the handle through `predict.reconciler` / `predict.sim` binds the stamp's interp-buffer term to the `Predict` lerp `delay` — the buffer you render at and the instant the server rewinds to come from **one number** and can't drift apart.
...
The rewind **timeline** must match how the **client displays** the target — "what you see is what you hit". Declare it per attach group with `mode`:
...
The shooter's *own* step can pre-judge the hit with the same timeline the server will use: `ctx.reckonTime` is the same value the server reads for that input, and `predict.valueAt(target, field, ctx.reckonTime)` samples the target where the rewind will place it — or batch the pose with `predict.readAt(target, fields, ctx.reckonTime)`, which runs one reckon integration per target instead of one per field. Pair it with an [optimistic event](/netcode/client-prediction#optimistic-events-defineevent) for instant feedback that auto-rejects if the server disagrees.

`ctx.reckonTime` is **always a usable instant** — a seq that wasn't stamped (the room doesn't rewind, the clock is still syncing) resolves to the live `serverNow()` automatically, mirroring the server side, where `channel.reckonTime` resolves unstamped reads to the room clock. There is no fallback to hand-write on either end. The rare consumer that needs the distinction — "was this seq actually stamped for lag comp?" — reads `ctx.lagCompActive`.
