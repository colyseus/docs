...
<SDKTabs>
    ...
    predict.AttachAll("players", new AttachOptions { Fields = new[] { "x", "y" } });
    var input = room.Input(cmd);
    var me = predict.Reconciler(self, new ReconcilerOptions<Player, MoveInput> {
        Input = input,   // binds the stamp to Delay = 100
        Step = (ctx, s, c) => ApplyInput(s, c, ctx.Dt),
    });
    ```
    </Tabs.Tab>
...
- **`renderDelay`**: pass it to `room.input()` only when you smooth remotes *outside* `Predict`; it must equal your actual interp buffer. The SDK adds the latency term itself. Pass only the buffer. The automatic value is the Predict's default lerp `delay`, read live, so a later `setDefaults({ delay })` moves the stamp too. In C#, `InputOptions.RenderDelay` is a `double?`: null binds, and any value, `0` included, is explicit.
...
Returns a [`RewindView`](#rewindview) of the world as that client last saw it. The method resolves the client's stamped render time and **clamps** it to `[now − maxRewindMs, now]` (anti-spoof / clock-skew bound). When the client hasn't synced yet, it **falls back to the live position**.
...
<Callout type="warning">
    **Don't break the delay pairing.** When the input handle is wired through `predict.reconciler`/`predict.sim`, the lerp `delay` and the rewind stamp share one source: nothing to keep in sync. That source is the Predict's *default* `delay`, so a per-attach `delay` that differs from it breaks the pairing too. If you pass `renderDelay` explicitly, it must equal the delay your remotes actually render at. Reverting remotes to `damped` (no fixed delay) silently degrades the rewind to an approximation.
</Callout>
