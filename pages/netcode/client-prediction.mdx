...
<Callout type="info">
    The construction options object is available in JavaScript, Haxe and Lua. The remaining ports take only the room and declare `mode`/`delay` per attach: `Predict.Get(room)` in C#, `Predict.get(room)` in Dart, and `Colyseus.Predict.of(room)` in GDScript.
</Callout>
