...
<SDKTabs>
    ...
    var predict = Predict.forRoom(room);
...
<Callout type="info">
    The construction options object is JavaScript-only. Ports take only the room and declare `mode`/`delay` per attach: `Predict.For(room)` (C#), `Predict.for_room(room)` (Lua), `Predict.forRoom(room)` (Haxe), `Colyseus.Predict.of(room)` (GDScript), `Predict.get(room)` (Dart).
</Callout>
