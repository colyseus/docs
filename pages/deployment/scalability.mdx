...
<Callout type="info">
    [Colyseus Cloud](https://colyseus.io/cloud-managed-hosting) does not require this configuration, as it automatically handles the public address for you.
</Callout>

<Tabs items={["Recommended Usage", "Raw usage"]}>
    <Tabs.Tab>

        ```ts {9-10} filename="app.ts"
        ...
        export default config({
            options: {
                ...
                publicAddress: `backend.yourdomain.com/${(process.env.PORT + Number(process.env.NODE_APP_INSTANCE))}`
            },
            // ...
        });
        ```
