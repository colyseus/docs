...
This change is additive. Existing two-argument handlers are unaffected. See [Room → Responding to a message](/room#responding-to-a-message-requestresponse) and [SDK → Request/Response](/sdk#requestresponse).
...
In 0.17, `room.setMetadata({ ... })` (and `room.setMatchmaking({ metadata: { ... } })`) would shallow-merge the provided fields into the existing metadata. In 0.18 these calls **replace** the entire metadata object, matching the behaviour of the `this.metadata = ...` setter and the rest of `setMatchmaking()`.
