...
- **Client:** `room.input()` and `Predict` (`reconciler` / `sim` for rollback, `attachAll` for remote smoothing).

These are additive — existing rooms are unaffected. See [Client Prediction](/netcode) for the full guide.
