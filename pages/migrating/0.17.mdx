...
Previously, registering a new handler for the same message type would replace the existing one. Starting from 0.17, `onMessage()` supports multiple handlers for the same message type — all registered callbacks will be invoked when a matching message is received.
