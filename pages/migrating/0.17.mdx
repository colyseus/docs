...
description: "Migration guide from Colyseus 0.16 to 0.17. Covers breaking changes, new APIs, and updated patterns."
---
...
On the frontend, replace `colyseus.js` with the `@colyseus/sdk` package:
...
The seat reservation returned by `matchMaker.reserveSeatFor(room)` has a new format. Update any code that manually accesses its `.room` object.
...
Alternatively, if you need to initialize the state in `onCreate()` (e.g., passing arguments to the state constructor), use the new generic type format:
...
There is no need to manually call `updateLobby()` after `.setMetadata()` anymore. The lobby update happens automatically.
...
#### `onMessage()` now supports multiple handlers per message type

Previously, registering a new handler for the same message type would replace the existing one. Starting from 0.17, `onMessage()` supports multiple handlers for the same message type. All registered handlers will be invoked when a matching message is received.

```typescript
this.onMessage("move", (client, message) => { /* handler 1 */ });
this.onMessage("move", (client, message) => { /* handler 2 */ });
// Both handlers will run when a "move" message is received.
```

If your code previously relied on re-registration to replace a handler, you must now explicitly unbind the previous one using the function returned by `onMessage()`:

```typescript
const unbind = this.onMessage("move", (client, message) => { /* ... */ });

// Later, replace the handler:
unbind();
this.onMessage("move", (client, message) => { /* new handler */ });
```
...
If your application was previously using codes in the 4xxx range for custom purposes (e.g., account banned, kicked), update your code to avoid conflicts. Choose values outside the codes listed above.

The [WebSocket specification](https://github.com/Luka967/websocket-close-codes) reserves 4000-4999 for application use, but Colyseus now uses part of this range internally.
</Callout>
