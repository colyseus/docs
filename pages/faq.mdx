...
If you define your state with [`@type()` decorators](/state/schema/decorators) and use TypeScript to target ES2022/ESNext, you need to provide `"useDefineForClassFields": false` on your tsconfig to make sure the decorators are defining property accessors properly. (The [`schema()` builder](/state/schema) does not need any tsconfig flags.)
