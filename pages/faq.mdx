...
The maximum number of concurrent users (CCU) a Colyseus server can handle will vary according to how CPU-intensive your game loop is, and how much traffic your server is sending back to the clients.
...
- **Profile your room code**: Avoid heavy computations in `onMessage` handlers. Offload intensive work to workers.
...
See our [Phaser tutorial on client-predicted input](/tutorial/phaser/client-predicted-input) for a practical implementation.
...
When a client disconnects (e.g., network issues), you can allow them to reconnect to the same room:
...
### Why does my WebSocket connection close immediately?

Common causes:
- **CORS issues**: Make sure your server allows connections from your client's origin
- **Wrong URL**: Verify the URL is correct (http:// for local, https:// for production)
- **Firewall/proxy**: Some networks block WebSocket connections
...
| Example: positions, scores | Example: chat, effects |
