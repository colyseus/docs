---
title: "FAQ"
description: "Frequently asked questions about Colyseus: performance, scaling, CCU limits, and common gotchas."
---
...
- **File descriptors**: Linux servers default to ~1024 open connections. This limit can be increased via `ulimit -n` (e.g., `ulimit -n 65535`), but ensure your hardware can handle the load.
...
[Optimizing State](/state/optimization) covers what a patch costs in bytes, and which field types, schema shapes and collection operations make it smaller.

### Does Colyseus help me with client-prediction?

Yes. Colyseus 0.18 ships first-class **client-side prediction and lag compensation**: `room.input()` / `Predict` on the client, `defineInput()` / `setFixedTimestep()` / `allowRewindState()` on the server. See [Netcode](/netcode) for the full stack. To learn the underlying concepts from scratch, the [Phaser tutorial](/learn/tutorial/phaser/client-predicted-input) builds them by hand.
...
- Server is under heavy load and by the time it can respond to a WebSocket connection request, the "seat reservation" code has already expired. [You may increase the seat reservation time limit](/matchmaker#reserve-a-seat-for) to remedy this.
...
If you define your state with [`@type()` decorators](/state/schema/decorators) and target ES2022/ESNext in TypeScript, set `"useDefineForClassFields": false` in your tsconfig. This flag makes sure the decorators define property accessors properly. (The [`schema()` builder](/state/schema) does not need any tsconfig flags.)
...
This error appears when the `target` in your `tsconfig.json` is below `es2015`. The official template uses `ESNext`:
...
Ensure these settings in your `tsconfig.json` (see [Decorators](/state/schema/decorators) for details):
...
If you set a breakpoint while the WebSocket connection is open, the connection closes automatically after 3 seconds due to inactivity. To prevent this, use `pingInterval: 0` during development:
