...
When `linkTo` is set, the panel makes each row clickable and navigates to that resource's show page, which is useful for "recent X" widgets that should deep-link back to the records.
