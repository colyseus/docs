...
| `mod` | **Only** on collections explicitly assigned via `db.moderation.assignMod()`: list, read, and update. No access at all outside assigned scopes, no create, no delete, and the roles table is always blocked. |
| `user` | Cannot use the panel — the UI requires an operator role (`admin`/`mod`) and returns 403. (Raw API list/read calls do pass the permission check.) |
...
<Callout type="info">
    The default `onResetRequest` forwards through `auth.settings.onForgotPassword` when configured, falling back to logging the URL via the admin logger — fine for local development, but you must override it before going to production. The request endpoint always returns `200` regardless of whether the email exists, so it cannot be used to enumerate users.
</Callout>
