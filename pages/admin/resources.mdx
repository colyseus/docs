...
Every drizzle table you pass to the admin panel becomes a browse-able **resource** with a full CRUD UI. The UI includes a paginated list with search/filter/sort, a detail view with related rows, create and edit forms, and per-row custom actions. The built-in tables from [`@colyseus/database`](/database) (users, configs, leaderboards, cloud saves, analytics events, roles, user notes, audit log) are registered automatically. You customize them and add your own via `defineAdminResource()`.
