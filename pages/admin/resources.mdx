...
| Roles | `colyseus_roles` | Always blocked for mods — modifying this would bypass the role system. |
...
Unless a table declares its own [policies](#policies) (only the audit log does by default), access follows the standard role rule everywhere: **admin** has full CRUD, **mod** gets list/read/update only on collections they're [scoped to](/admin/authentication#scoping-a-moderator-to-specific-resources), and **user**-role accounts have raw-API list/read only (the panel UI itself requires an operator role).
