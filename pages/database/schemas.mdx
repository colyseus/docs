...
The examples import column builders from `drizzle-orm`. Install it from the `rc` dist-tag, as described under [Drizzle version](/database#drizzle-version).
...
<Callout type="warning">
    The PostgreSQL factory's third argument currently fails type-checking. `tsc` rejects `.on(t.handleLower)` with "Property 'dimensions' is missing", although the code runs. Until the fix ships, use the [hand-rolled `pgTable` form](#hand-rolled-tables-with-columns), which type-checks. SQLite is not affected.
</Callout>
...
With `drizzle-kit@rc` installed (see [Drizzle version](/database#drizzle-version)), generate the migration:
