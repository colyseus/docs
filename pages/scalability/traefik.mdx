---
title: "Traefik Load Balancer"
description: "Scale Colyseus across multiple processes using Traefik as a reverse proxy and load balancer."
---
...
The `@colyseus/traefik` module allows you to dynamically expose Colyseus servers to [Traefik](https://traefik.io/) as a load balancer.
...
- [RedisPresence](/server/presence#redis-presence) for presence management.
...
<Tabs items={["Redis Provider (Recommended)", "HTTP Provider"]}>
    <Tabs.Tab>
        ...
        ```typescript {3} filename="app.config.ts"
        await exposeServerToTraefik({
            server,
            provider: "redis",
