import { javascript, typescript, react, unity, defold, construct3, cocos, haxe, discord, wechat, gamemaker, godot, monogame, flutter, swift, c } from '../../components/icons/platforms'

export default {
    "typescript": { title: <span>{typescript({ width: '19px', marginRight: '2px' })} TypeScript</span> },
    "react": { title: <span>{react({ width: '19px', marginRight: '2px' })} React</span> },
    "unity": { title: <span>{unity({ width: '19px', marginRight: '2px' })} Unity</span> },
    "monogame": { title: <span>{monogame({ width: '19px', marginRight: '2px' })} MonoGame</span> },
    "godot": { title: <span>{godot({ width: '19px', marginRight: '2px' })} Godot</span> },
    "defold": { title: <span>{defold({ width: '19px', marginRight: '2px' })} Defold</span> },
    "construct3": { title: <span>{construct3({ width: '19px', marginRight: '2px' })} Construct 3</span> },
    "gamemaker": { title: <span>{gamemaker({ width: '19px', marginRight: '2px' })} GameMaker</span> },
    "cocos": { title: <span>{cocos({ width: '19px', marginRight: '2px' })} Cocos Creator</span> },
    "haxe": { title: <span>{haxe({ width: '19px', marginRight: '2px' })} Haxe</span> },
    "flutter": { title: <span>{flutter({ width: '19px', marginRight: '2px' })} Flutter</span> },
    "swift": { title: <span>{swift({ width: '19px', marginRight: '2px' })} Swift</span> },
    "native-sdk": { title: <span>{c({ width: '19px', marginRight: '2px' })} Native SDK (C)</span> },
    "discord-activity": { title: <span>{discord({ width: '19px', marginRight: '2px' })} Discord Activity</span> },
    "wechat": { title: <span>{wechat({ width: '19px', marginRight: '2px' })} WeChat</span> },
}