...
<Tabs.Tab>
Download the release binaries from [GitHub Releases](https://github.com/colyseus/colyseus/releases?q=%22%40colyseus%2Fsdk%22&expanded=true) and include them in your project:
...
Provide the `typeof server` type as a generic to the client. This generic allows the client to infer both the state type and the message types for the room.
...
<Callout type="info">
`import type` only imports type information at compile time, so no server code is included in your client bundle.
</Callout>
...
Use `import type` to import the state type and pass it as a generic to `joinOrCreate<T>()`. This generic gives you **type inference** for `room.state` and state callbacks.
...
Use `import type` and pass the type as a generic to `joinOrCreate<T>()`. This generic gives you **type inference** for `room.state` and state callbacks.
...
If you define your state with the [`@type()` decorator style](/state/schema/decorators) and target ES2022 or ESNext, ensure your `tsconfig.json` includes:
...
These flags are required only for `@type()` decorators. The recommended [`schema()` builder](/state/schema) works without them.
...
- [State Sync Callbacks](/state/callbacks) - Listen for state changes
