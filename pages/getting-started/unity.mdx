...
A resolved join does **not** mean the state has been synchronized yet: `room.State` is instantiated when you join, but its collections (`players`, nested refs, …) are only populated by the first full state sync, which arrives a moment later. Code that reads `room.State` right after joining can find `null` maps.

`WaitForFirstState()` covers the gap — it is the C# analogue of the JavaScript SDK's `room.onStateChange.once(...)`:
...
The same `schema-codegen` command also generates C# classes for any TypeScript interface containing `"Message"` in its name. This allows you to use strongly-typed message objects on the frontend.
