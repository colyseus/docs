...
A resolved join does **not** mean the state has been synchronized yet. `room.State` is instantiated when you join, but its collections (`players`, nested refs, …) are only populated by the first full state sync. That sync arrives a moment later. Code that reads `room.State` right after joining can find `null` maps.
...
<Callout type="info">
    Registering `Callbacks` (as in the Quick Example above) is safe *before* the first sync. Callbacks are driven by the decoder and fire as the initial state decodes. `WaitForFirstState()` is for code that reads `room.State` directly.
</Callout>
