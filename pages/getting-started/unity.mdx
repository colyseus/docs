...
<Callout type="warning">
    Never poll for state with `Task.Delay` on **WebGL** builds. WebGL has no thread-pool timers, so `Task.Delay` never resumes and the wait hangs forever. `WaitForFirstState()` is timer-free: it completes on the message pump and works on every platform, including WebGL.
</Callout>
