...
<Callout type="info">
    You may define the same room handler multiple times with different `options`. When [Room#onCreate()](/room/lifecycle#on-create) is called, the `options` will contain the merged values you specified on [Server#define()](/server/#define-room-type) with the options provided by the client SDK.
</Callout>
...
<Callout type="warning">
    Use these events for logging and monitoring purposes only. It is not encouraged to manipulate a room's state through these events. Use the [Room's Lifecycle Events](/room/lifecycle) in your room handler instead.
</Callout>
