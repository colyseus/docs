...
<Callout type="info">
    You may define the same room type multiple times with different `options`. When [Room#onCreate()](/room/lifecycle#on-create) is called, the `options` will contain the merged values you specified on [Server#define()](/server/#define-room-type) with the options provided by the client SDK.
</Callout>
