...
The plugin's message handler keys also flow through to the SDK's `room.send(...)` / `room.onMessage(...)` for typed client-side use — see [Send and Receive Messages](/sdk/messages) for the client-side view.
