...
All plugins above ship with the `colyseus` package, with no extra dependency to install. The exception is `GeoIPPlugin`, which lives in its own package ([`@colyseus/geoip`](/room/plugins/geoip)) because it pulls in the MMDB reader and database files.
...
- If the **room** defines the same message key as a plugin, the room wins: the plugin's handler is silently dropped. This behavior is the documented escape hatch for _"use the plugin but override one message"_.
