...
At runtime the plugin's message handlers are fully merged into the room. For **client-side typing**, though, a `protected messages` map (the visibility this page recommends) does not surface through the SDK's `room.send(...)` / `room.onMessage(...)` type extraction — TypeScript's structural checks skip protected members. If typed client access to a plugin's messages matters more than autocomplete hygiene, declare that plugin's `messages` as `public`.
