---
title: "Relay Room"
description: "The built-in RelayRoom broadcasts client messages to everyone else with minimal backend state — for simple multiplayer without server authority."
---
...
const server = defineServer({
  rooms: {
    your_relayed_room: defineRoom(RelayRoom, {
      maxClients: 4,
      allowReconnectionTime: 40 // seconds — values above 40 are clamped to 40
    })
  }
})
```
...
const callbacks = Callbacks.get(relay);
...
// (only relevant if you provided `allowReconnectionTime` in the backend)
//
callbacks.onAdd("players", (player, sessionId) => {
