...
**This is signaling only.** It handles the SDP/ICE exchange needed to establish WebRTC connections. The actual audio/video/data streams flow directly between peers. Colyseus state synchronization continues to work through the server as usual.
...
The plugin tracks which clients have explicitly opted into the mesh, so spectators and other non-WebRTC clients in the same room don't show up in peer events.
