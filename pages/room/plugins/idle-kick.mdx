...
<Callout type="info">
  `this` inside `onKick` is the plugin instance. To call back into the room, capture it from a closure or use the plugin handle (`this.plugins.idle`) from inside your own room methods instead.
</Callout>

### Custom close code

If you'd rather signal a domain-specific reason (e.g., to differentiate from a generic 1000 disconnect), use a custom code in the WS application range (`4000`–`4999`):
...
- **Any inbound frame refreshes activity** — including SDK keepalive PINGs. This is by design: an idle client whose socket is still healthy is preferable to a zombie connection that gets force-closed. If you need stricter "user-payload-only" semantics, manage your own timestamps from your message handlers.
