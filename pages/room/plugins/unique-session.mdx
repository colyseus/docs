...
plugins = definePlugins([
  new UniqueSessionPlugin({
    ...
    conflictsWith: (existing) =>
      existing.room?.processId === matchMaker.processId,
  }),
]);
```
...
plugins = definePlugins([
  new UniqueSessionPlugin({
    ...
    // playing meaningfully — let them re-queue elsewhere.
    conflictsWith: (existing) =>
