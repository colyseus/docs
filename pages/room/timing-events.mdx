---
title: "Timing Events"
description: "Use delayed actions, intervals, and the built-in clock in Colyseus rooms for game loops and scheduled events."
---
...
it's recommended to use the [`this.clock`](/room#clock) methods,
from your `Room` instance.

<Callout type="info">
    All intervals and timeouts registered on [`this.clock`](/room#clock) are cleared automatically when the `Room` is disposed.
</Callout>
...
<Callout type="info">
    See [Room#setSimiulationInterval()](/room#set-simulation-interval) for more details.
</Callout>
