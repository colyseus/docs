...
description: "Handle uncaught exceptions in Colyseus rooms to prevent crashes and implement custom error recovery."
---
...
By default, if an uncaught exception occurs inside a room, the server is shut down ungracefully. This shutdown disconnects all clients, and the server will restart or stop running (depending on how it's configured).
...
- `onDispose()`
- `onMessage()`
- `setTimestep()` / `setFixedTimestep()` (and the deprecated `setSimulationInterval()` alias)
- `clock.setTimeout()`
- `clock.setInterval()`
...
- `client` - the client that is leaving the room.
- `consented` - the close code associated with the client leaving the room.
...
// ...
    onUncaughtException (err: Error, methodName: string) {
        if (err instanceof OnLeaveException) {
            err.client; // the client that is leaving the room.
            err.consented; // the close code associated with the client leaving the room.
            err.cause; // original unhandled error
            err.message; // original error message
        }
    }
...
### OnDropException / OnReconnectException

When the exception is thrown inside `onDrop` (properties: `client`, `code`) or `onReconnect` (property: `client`).

<Callout type="warning">
    In 0.18.1 these two classes are raised internally but not yet exported from the `colyseus` package, so an `instanceof` check isn't possible. Inspect the `methodName` argument (`"onDrop"` / `"onReconnect"`) instead.
</Callout>
...
When the exception is thrown inside the `setTimestep` or `setFixedTimestep` callback (`methodName` is `"setTimestep"` / `"setFixedTimestep"`). `SimulationIntervalException` remains as a deprecated alias.

```ts filename="MyRoom.ts"
import { TimestepException } from "colyseus";
// ...
    onUncaughtException (err: Error, methodName: string) {
        if (err instanceof TimestepException) {
            err.cause; // original unhandled error
            err.message; // original error message
        }
    }
