...
room.reconnection.enabled = true;          // Enable/disable auto-reconnection (default: true)
...
|--------|---------|-------------|
| `enabled` | `true` | Set to `false` to disable automatic reconnection entirely. When disabled, a dropped connection triggers `onLeave` immediately with the original close code. |
...
| `retryCount` | 0 | Current number of reconnection attempts (read-only) |
| `isReconnecting` | false | Whether the room is currently reconnecting (read-only) |
