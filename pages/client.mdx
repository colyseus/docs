...
Every client-side SDK provides the same set of methods and events to interact with the server-side.

- [Client](#client) - The SDK entrypoint
- [Room](#room) - Room connection.
- [HTTP](#http-client) - HTTP requests.
- [Auth](/auth/module#client-side-api) _(Optional)_ - Authentication module.

## Client

The `Client` instance is the entrypoint to connect to the server-side. It is used to create or join rooms, and perform HTTP requests.
...
The current room's state. This variable is always synched with the latest `state` from the server-side. To listen for updates on the whole state, see [`onStateChange`](#onstatechange) event.
