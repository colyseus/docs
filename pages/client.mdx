...
<Callout type="info">
    The `client.auth.token` property is sent automatically as `Authorization` header in all HTTP requests. See [Authentication → Auth Token](/auth/auth-token) for more details.
</Callout>
