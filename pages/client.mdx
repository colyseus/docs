...
For fine-grained control over state updates, see [Client SDK → State Sync Callbacks](/client/state-sync-callbacks).
...
For fine-grained control over state updates, you should use the [State Sync Callbacks](/client/state-sync-callbacks).

<Callout type="info">
    Check out the [Client SDK → State Sync Callbacks](/client/state-sync-callbacks) section for more details.
</Callout>
...
- [State Sync Callbacks](/client/state-sync-callbacks) - Listen for state changes on the client
