...
<Callout type="info">
    **Consuming the seat reservation** - You can use [`consumeSeatReservation()` from the frontend](/sdk/#consume-seat-reservation) to join the room by its reserved seat.
</Callout>
...
Find an available room the same way `join()`/`joinOrCreate()` do: rooms flagged [`locked` or `private`](/room#matchmaking-properties) are skipped (`unlisted` rooms still match).
