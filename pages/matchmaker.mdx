...
Find an available room the same way `join()`/`joinOrCreate()` do: rooms flagged [`locked` or `private`](/room/visibility#matchmaking-properties) are skipped (`unlisted` rooms still match).
