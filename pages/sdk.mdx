...
The promise rejects (or the callback receives an `error`) when the handler throws, no handler is registered for that type, the connection closes first, or no reply arrives within the timeout. The default timeout is `room.requestTimeout` (10000 ms).
...
- [Client Prediction](/netcode) - Predict local input and reconcile to server state
