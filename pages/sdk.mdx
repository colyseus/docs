...
The SDK automatically attempts to reconnect when the connection is unexpectedly dropped. Configure this behavior using `room.reconnection`.
...
<Callout type="info">
    **Close codes that trigger `onDrop`:**
    ...
    - `4050` - May try reconnect
</Callout>

#### On Reconnect Event

Triggered when the client successfully reconnects after a connection drop. Buffered messages are automatically sent to the server.
...
<Callout type="info">
    The room state remains intact during reconnection. No state re-synchronization is needed as the serializer state is preserved on the client.
</Callout>

### Manual Reconnection

For more control, you can manually reconnect using a cached reconnection token.
