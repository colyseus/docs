...
For fine-grained control over state updates, see [Client SDK → State Sync Callbacks](/sdk/state-sync-callbacks).
...
For fine-grained control over state updates, you should use the [State Sync Callbacks](/sdk/state-sync-callbacks).

<Callout type="info">
    Check out the [Client SDK → State Sync Callbacks](/sdk/state-sync-callbacks) section for more details.
</Callout>
