---
title: "Presence"
description: "The Presence API enables communication between rooms and Node.js processes in Colyseus: LocalPresence for development, RedisPresence for scaling, and pub/sub methods."
---
import { Callout, Cards, Tabs } from "nextra/components";

# Presence

The Presence API is responsible for inter-process communication (IPC) when scaling Colyseus across multiple processes or machines. This API provides pub/sub messaging and shared key-value storage that all server instances can access.
...
The default presence used by Colyseus is `LocalPresence`, which stores all data in memory and only works within a single process. This presence is suitable for development and small-scale single-process applications.

---

### Redis Presence

The `RedisPresence` uses Redis for pub/sub messaging and shared storage using [ioredis](https://github.com/redis/ioredis). This presence is required for multi-process or distributed deployments.
...
Alternatively, you can access `matchMaker.presence` from anywhere by importing the [Matchmaker API](/matchmaker/):
