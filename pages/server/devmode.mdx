...
description: "Colyseus development mode (devMode) restores rooms and state across server restarts while you iterate on room code locally."
---
...
<Callout type="info">
    The frontend code is **not reloaded**, only the connection is re-established. If you want HMR for both your frontend and server code from a single Vite project, see the [Vite Plugin](/server/vite).
</Callout>
...
The `devMode` is disabled by default and it can be enabled via [Server option](/server#development-mode):
