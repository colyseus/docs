...
description: "How Colyseus shuts down gracefully on SIGTERM/SIGINT — draining rooms via onBeforeShutdown, and customizing or disabling the shutdown routine."
---
import { Callout, Steps, Tabs } from "nextra/components";

# Graceful Shutdown Process

Colyseus listens for `SIGTERM`, `SIGINT`, and `SIGUSR2` signals (and `uncaughtException`) to gracefully shut down the process.
...
<Steps>
    ...
    The custom `server.onBeforeShutdown()` is called, if defined.
    ...
    ### Process is excluded from matchmaking.
    ...
    <Callout type="info">
        See [Room API → Lock Room](/matchmaker/visibility#lock-room)
    </Callout>
    ...
    <Callout type="info">
        See [Room API → On Before Shutdown](/room/lifecycle#on-before-shutdown)
    </Callout>
    ...
    <Callout type="info">
        See [Matchmaker → Get Local Room Count](/matchmaker#get-local-ccu)
    </Callout>
    ...
    The custom `server.onShutdown()` is called, if defined.
...
You may use `async` functions or return a `Promise` to perform asynchronous operations on `onLeave` and `onDispose` methods, as well as `server.onBeforeShutdown()` and `server.onShutdown()`.
