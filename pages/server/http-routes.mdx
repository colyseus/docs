...
<Callout type="info">
    When [`@colyseus/database`](/database) is in use, `auth.middleware()` is **revocation-aware** automatically: `db.auth.bumpTokenVersion(userId)` invalidates live JWTs and bans gate at the token layer — no per-route check needed. See [Database → Authentication](/database#authentication-built-in-colyseusauth-integration).
</Callout>
