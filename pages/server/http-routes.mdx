...
<Callout type="info">
    When [`@colyseus/database`](/database) is in use, `auth.middleware()` is **revocation-aware** automatically: `db.auth.ban(userId)` and `db.auth.bumpTokenVersion(userId)` both invalidate live JWTs at the token layer — no per-route check needed. See [Database → Authentication](/database#authentication-built-in-colyseusauth-integration).
</Callout>
...
[Monitor](/tools/monitor) in production — both accept a `use:` array:
