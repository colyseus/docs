---
title: "HTTP Routes"
description: "Define custom HTTP routes on a Colyseus server, protect them with middleware, and configure CORS headers."
---
...
For JWT-based auth, [`@colyseus/auth`](/auth/module) exports `auth.middleware()`, a middleware that verifies the `Authorization: Bearer <token>` header and exposes the decoded payload on `ctx.context.auth`. It's the canonical way to gate an endpoint behind a logged-in user.
...
<Callout type="info">
    When [`@colyseus/database`](/database) is in use, `auth.middleware()` is **revocation-aware** automatically: `db.auth.ban(userId)` and `db.auth.bumpTokenVersion(userId)` both invalidate live JWTs at the token layer (no per-route check needed). See [Database → Authentication](/database/authentication).
</Callout>

See [Authentication → HTTP Middleware](/auth/http) for the dual Express/`createEndpoint` modes, and [Auth Module](/auth/module) for backend setup.

### Basic Authentication

`basicAuth` is a built-in middleware for HTTP Basic Auth. Drop it into any
endpoint's `use:` array to gate it behind the browser's native credentials
prompt.

```ts
import { createEndpoint, basicAuth } from "colyseus";

export const adminStats = createEndpoint("/admin-stats", {
    method: "GET",
    use: [ basicAuth({ users: { admin: "s3cret" } }) ],
}, async (ctx) => {
    return { onlineUsers: 123 };
});
```

A common use is protecting the [Playground](/tools/playground) and
[Monitor](/tools/monitoring) in production. Both accept a `use:` array:
...
| Option | Description |
|--------|-------------|
| `users` | `Record<string, string>` mapping username → password. The common static case. |
| `validate` | `(username, password) => boolean \| Promise<boolean>`: custom/DB-backed check. Takes precedence over `users`. |
| `realm` | Label shown in the browser prompt. Default `"Restricted"`. |

<Callout type="info">
    On a missing or invalid credential the middleware responds `401` with a
    `WWW-Authenticate: Basic` header, so browsers show the native login dialog.
    Passwords are compared in constant time. You must provide at least one of
    `users` or `validate`.
</Callout>
...
| `metadata.scope` | `"rpc"` (default): available to RPC client. `"server"`: routed and callable directly, hidden from the typed RPC client. `"http"`: routed only. |
...
| `ctx.setSignedCookie(name, value, secret, options?)` | Set a signed cookie. |
| `ctx.getSignedCookie(name, secret)` | Read a signed cookie. |
