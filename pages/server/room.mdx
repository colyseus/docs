...
- `options`: merged values specified on [Server#define()](/server/#define-roomname-string-room-room-options-any) with the options provided by the SDK.
- _(optional)_ `auth`: auth data returned by [`onAuth`](#onauth) method.
...
An optional hook to reprocess/restore data which was returned and stored from the previous hook [`onCacheRoom`](/server/room/#oncacheroom-devmode) when [Development Mode](devmode) is enabled.
...
<Callout type="info">
    Use `room.send()` from the client SDK to send messages to the server.
</Callout>
...
Private Rooms are not listed during [`getAvailableRooms()`](/client/#get-available-rooms) method.
