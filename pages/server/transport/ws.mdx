...
description: "The default Colyseus WebSocket transport (ws): options, ping/pong heartbeats, and customization."
---
...
<Callout type="info">
    The underlying library is [`websockets/ws`](https://github.com/websockets/ws). The `ws` package is a fast and thoroughly tested WebSocket client and server for Node.js.
</Callout>

## Example

```ts {2,7-11} filename="app.config.ts"
...
<Tabs items={["Example", "Example + Express"]}>
    ...
    const app = express();
    const httpServer = createServer(app); // create the http server manually
...
[verifyClient](#optionsverifyclient) covers the accept-or-reject case on this
transport. `beforeUpgrade` answers with a full `Response`, and behaves the same
way on every WebSocket transport.
...
<Callout type="info">
    See [Node.js documentation on --max-http-header-size](https://nodejs.org/api/cli.html#--max-http-header-sizesize) for more information.
</Callout>
