...
The Colyseus Vite plugin runs your game server **inside** Vite's dev server using Vite's [Environment API](https://vite.dev/guide/api-environment). A single `vite` process serves your frontend and hosts the Colyseus server, with [Hot Module Replacement](https://vite.dev/guide/features#hot-module-replacement) (HMR) for both client components **and** server-side room definitions, with no process restart between edits.
