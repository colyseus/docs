...
The Colyseus Vite plugin runs your game server **inside** Vite's dev server using Vite's [Environment API](https://vite.dev/guide/api-environment). A single `vite` process serves your frontend and hosts the Colyseus server, with [Hot Module Replacement](https://vite.dev/guide/features#hot-module-replacement) (HMR) for both client components **and** server-side room definitions — no process restart between edits.
...
In standalone dev mode the plugin attaches the WebSocket transport to Vite's own HTTP server. When you run Vite in [middleware mode](https://vite.dev/guide/ssr) — embedding it inside your own Express/HTTP server — that server owns the socket, so you must pass it explicitly:
