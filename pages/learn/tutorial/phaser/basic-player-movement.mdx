---
title: "Part 1: Basic Player Movement"
description: "Part 1 of the Phaser + Colyseus tutorial: set up the server and synchronize basic player movement."
---
...
To access the frontend project from your web browser, let's start `parcel` in another terminal tab. Parcel builds and serves the frontend files:
...
When you refresh your browser now, your client is going to establish a connection with the server. The server then creates the room `my_room` on demand for you.
...
The state mutations we've done on the backend **can be observed** on the frontend. That's what we're going to do in the next sections.
...
![Player Sprite](https://cdn.jsdelivr.net/gh/colyseus/tutorial-phaser@master/client/dist/assets/ship_0001.png)
...
After a connection with the room has been established, the frontend can start listening for state changes. The frontend then creates a visual representation of the data in the server.
...
For the visual representation, we need to add a Phaser GameObject for each joining player. We also keep a local reference to each GameObject, keyed by `sessionId`, so we can operate on them later:
...
For this particular example, we are going to send the player's input at every tick. This approach is a requirement for the client-side prediction technique applied by the end of this tutorial.
