---
title: "Part 2: Linear Interpolation"
description: "Part 2 of the Phaser + Colyseus tutorial: smooth remote player movement with linear interpolation."
---
...
<Callout type="info">
    This tutorial implements interpolation **by hand** to teach the concept. For the built-in 0.18 API that smooths remote entities for you, see [Client Prediction → Remote entities](/netcode/client-prediction#remote-entities).
</Callout>
...
On [Part 1](basic-player-movement) of this tutorial, we updated the player's visual representation at the same instant as state updates are received from the server, using `callbacks.onChange()`:
...
By doing so, the result is "choppy" because the frequency of updates received from the server is lower than the frequency the frontend render each frame.

Colyseus sends state updates to the client at every **50ms** (_20fps_) by default, whereas the frontend re-renders at every **16.6ms** (_60fps_).
