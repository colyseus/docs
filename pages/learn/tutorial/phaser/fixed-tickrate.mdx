...
Let's update the server's `Player` definition to hold a queue of inputs. The `.noSync()` modifier keeps the field on the instance without ever synchronizing it to clients:
