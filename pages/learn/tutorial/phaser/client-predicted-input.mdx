---
title: "Part 3: Client Predicted Input"
description: "Part 3 of the Phaser + Colyseus tutorial: apply input locally before the server confirms it (client prediction)."
---
import { Callout, Cards, Tabs } from "nextra/components";
import { DownloadIcon, LinkIcon } from '@primer/octicons-react'

# Phaser #3: Client Predicted Input

<Callout type="info">
    This tutorial implements prediction **by hand** to teach the concept. For the built-in 0.18 API (`room.input()` + `Predict.reconciler` with rollback and reconciliation), see [Client Prediction](/netcode/client-prediction).
</Callout>
...
- Update **CURRENT** player position immediately at the rate of the frontend's update loop
...
Client-side prediction is a technique that reduces the _perceived_ delay of the local player. The client gives immediate feedback while its inputs and/or messages haven't reached the server yet.
...
We need to implement in the frontend the same logic we already have on the backend for player movement.

Instead of waiting for the server's acknowledgement, we apply the position change locally at the same instant we send the input to the server:
...
From the backend, in the `app.config.ts` file, add the following:
