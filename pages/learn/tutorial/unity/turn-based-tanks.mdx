...
description: "Build a turn-based multiplayer tanks game with Unity and Colyseus."
---
...
![Lobby](/tutorial/unity/turn-based-tanks/WeaponFired.PNG)
...
If you are running a local server, the default settings should be sufficient. If you wish to host a server, change the **Colyseus Server Address** and **Colyseus Server Port** values accordingly.

## Demo Overview

### Room Metadata

This demo makes use of the room&#39;s metadata to track the players in the game with their username. When a player joins or creates a room, their username is stored in a property called either `team0` or `team1`. `team0` represents the player that created the room. `team1` represents the player that joined an available room to challenge the creator.
``` javascript
this.metadata.team0
this.metadata.team1
...
The usernames set in the metadata are then used to filter the available rooms displayed in the lobby. Within the lobby, users see the rooms they have created, plus any room that is still waiting for a challenger to join. Rooms that you have not created and have two players will not be shown in the lobby.
...
![Lobby](/tutorial/unity/turn-based-tanks/Rooms.PNG)

### Keeping the Room Alive

To make this demo an asynchronous turn based game, we need to keep the room alive even after both players have left. The room is kept alive by setting the `autoDispose` flag to false (see the `onCreate` hook in the TanksRoom server code).
...
When users disconnect from the room a check is performed to look if there are no more users connected to the room. When no more users are connected to the room the timestep effectively gets paused by setting the delay to a high value. In this case the value is a little more than 24 days.
``` javascript
...
private setServerPause(pause: boolean) {
    ...
    if(pause) {
        this.setTimestep(dt => this.gameLoop(dt), this.pauseDelay);
    }
    else {
        // Set the timestep callback
        this.setTimestep(dt => this.gameLoop(dt));
    }
...
When users rejoin a room that has been paused, the timestep is restored.
``` javascript
...
- You can leave a room at any point and later return to a game in progress. The game resumes where it left off.
...
- You can leave the room at any time using the Exit option in the ESC menu, or Surrender the game to your opponent.
...
- A game ends when a player&#39;s tank is destroyed or someone surrenders. A game over menu then shows a win/loss message, with options to request a rematch or quit the game. If the other player requests a rematch before you leave, a message will be displayed on the game over menu.
...
![Lobby](/tutorial/unity/turn-based-tanks/GameplayWithLabels.png)
![Lobby](/tutorial/unity/turn-based-tanks/GameOver.PNG)
