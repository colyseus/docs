---
title: "PlayCanvas"
description: "Build a real-time multiplayer game with PlayCanvas and Colyseus."
---
import { Callout, Cards, Tabs } from "nextra/components";
import { LinkIcon } from '@primer/octicons-react'

# PlayCanvas: Real Time Multiplayer Tutorial
...
- [<LinkIcon/> PlayCanvas Project (Frontend)](https://playcanvas.com/project/859259/overview/colyseus-demo)
- [<LinkIcon/> Colyseus TypeScript Project (Backend)](https://github.com/colyseus/tutorial-playcanvas-server)
...
This external script makes the `Colyseus` [JavaScript / TypeScript SDK](/getting-started/typescript) available for our PlayCanvas scripts.
...
NetworkManager.prototype.initialize = function () {
  ...
  //
  this.app.colyseus = new Colyseus.Client("http://localhost:2567");
...
<Callout type="info">
    Note that we're using the local `http://localhost:2567` endpoint here. You need to [deploy your server](/deployment) to the public internet in order to play with others online. You can also use [Glitch](https://glitch.com/edit/#!/tutorial-playcanvas-server) to host your server publicly.
</Callout>

When you **"Launch"** your PlayCanvas project now, your client establishes a connection with the server. The server then creates the room `my_room` on demand.
...
![Plane](/tutorial/playcanvas/plane.jpg)
...
![Player](/tutorial/playcanvas/player.png)
...
```typescript
//
// In order to attach callbacks to the state, we need to use the `Callbacks.get()` method
//
...
We are going to listen for the "mouse down" event. Using [ray cast](https://developer.playcanvas.com/en/user-manual/physics/ray-casting/), we determine the exact `Vec3` position the player should move towards. We then send that position as a message to the server.
...
![monitor](/tutorial/playcanvas/monitor.png)
