---
title: Hide and Seek Tech Demo
description: "Build a multiplayer hide-and-seek game with the Babylon.js Editor and Colyseus."
---
...
This demo, "Hide and Seek", is a simple game where players are randomly assigned the role of Hider (ghost) or the lone Seeker (pumpkin head). In order for the Hiders to win they need to evade the Seeker until time runs out.
...
![Hide and Seek Demo](/examples/babylonjs/hide-and-seek/title-screenshot.png)
...
<Callout type="info">
    - Install the server before you attempt to run or open the project in the editor, or compilation errors will result. The errors happen because client code references class types from the server code, for development convenience.
...
#### Colyseus Client SDK
You do not need to install the Colyseus client SDK manually for this demo. The Babylon.js Editor installs it automatically, because it is listed as a dependency in the `package.json` of the project workspace. If needed, manual instructions on how to install the Colyseus client SDK in the Babylon.js Editor can be found [here](/getting-started/typescript).

#### Colyseus Server Settings
All server settings are defined in `.env` files located in the Babylon.js Editor workspace here:

![Server Settings](/examples/babylonjs/hide-and-seek/server-settings.png)
...
## Playing the Demo
![Editor](/examples/babylonjs/hide-and-seek/editor.png)
...
By default a minimum of three players is required to start a match, but you can adjust that, as noted in the section below. If necessary you can open multiple client instances to satisfy the player requirement.

<Callout type="info">
    - For better performance, try running the client in your browser rather than the editor's integrated browser (right click the "Run" button)

    ![Run](/examples/babylonjs/hide-and-seek/run.png)

    - If you cannot reach the lobby UI confirm your local server is working properly and check the editor logs for errors.
</Callout>

![Gameplay](/examples/babylonjs/hide-and-seek/gameplay.png)
...
- There are various traps spread out over the arena that betray a Hider’s position to the Seeker
...
- A Hider is captured when they enter the Seeker's FOV, or when they get within about a unit of the Seeker in any direction
...
![Server Settings](/examples/babylonjs/hide-and-seek/config.png)
...
There are two values involved with rescuing Hiders. `rescueDistance` controls the max distance another Hider can be in order to rescue their friend. `rescueTime` is the time, in milliseconds, it takes to rescue a Hider.
...
![Countdowns](/examples/babylonjs/hide-and-seek/countdowns.png)
...
- `initializedCountdown`: a brief pause after the server initializes the room for a round of play. This pause gives clients time to receive updated schemas and handle spawning before the prologue state.
