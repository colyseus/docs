---
title: Basics
description: "Introduction to building multiplayer games with Babylon.js and Colyseus: server setup, room connection, and state synchronization."
---
import { Callout, Cards, Tabs } from "nextra/components";
import { DownloadIcon, LinkIcon } from '@primer/octicons-react'

# Babylon.js: Real Time Multiplayer Tutorial
...
npm create colyseus-app@latest ./babylonjs-multiplayer-server
```
...
For simplicity's sake, the examples on this guide are using the [Babylon.js Playground](https://doc.babylonjs.com/toolsAndResources/tools/playground). Although the full source-code available for download uses [NPM + Webpack](https://doc.babylonjs.com/divingDeeper/developWithBjs/npmSupport).

In the Playground, we inject the Colyseus JavaScript SDK manually through a `<script>` tag created via code. This technique is described in [_"Using External Assets In The Playground"_ → _"Javascript files"_](https://doc.babylonjs.com/toolsAndResources/tools/playground/externalPGAssets#javascript-files).
...
In a real-world scenario, please follow the [official Colyseus documentation](/getting-started/typescript) on how to include the Colyseus JavaScript SDK.
...
<Callout type="info">
    Note that we're using the local `http://localhost:2567` endpoint here. You need to [deploy your server](/deployment) to the public internet in order to play with others online.
</Callout>
...
When you run your BabylonJS project now, your client establishes a connection with the server. The server then creates the room `my_room` on demand.
...
The state mutations we've done in the backend **can be observed** in the frontend. We'll do that in the next section.
...
Once connected to the room, the frontend can start listening for state changes and build a visual representation of the server's data.
...
For the visual representation, we need to clone the "Player" object. Keep a local reference to each clone, keyed by `sessionId`, so we can operate on them later:
...
We are going to use the `Scene.onPointerDown` event to determine the exact `Vector3` position the player should move towards. We then send that position as a message to the server.
...
<Callout type="info">
    See [Client SDK → State Sync Callbacks](/state/callbacks)
</Callout>
...
To enable position interpolation, we're going to use the [Render Loop](https://doc.babylonjs.com/features/featuresDeepDive/animation/render_frame_animation) and the [`Scalar.Lerp()`](https://doc.babylonjs.com/typedoc/classes/BABYLON.Scalar#Lerp) method.

Instead of updating the player position directly (as in [previous section](#updating-players-visual-representation)), we are going to cache the next position. During the Render Loop, we constantly interpolate each player position towards it:
