...
On the frontend, you can change the length of time a message shows for by changing the public `messageShowTime` variable on `ChatManager.ts` which will then be sent to the server when Joining/Creating a room in `MMOManager.ts`:
