...
The room's lifecycle events are called automatically. [`async`/`await`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function) is supported on every lifecycle event.
...
The `client` instance from the backend is responsible for the **transport** layer between the server and the client. It should not be confused with the [`Client` from the frontend SDK](/client/), as they have completely different purposes.
...
<Callout type="info">
    In the frontend, you can find the [`sessionId` in the `room` instance](/client/#sessionid-string).
</Callout>
...
<Callout type="info">
    [See how to handle `onMessage` from the frontend SDK](/client/#on-message)
</Callout>
...
<Callout type="info">
    This will trigger [`room.onLeave`](/client/#on-leave) event on the frontend.
</Callout>
