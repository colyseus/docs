...
Use `this.onMessageBytes()` to receive raw `Uint8Array` messages sent via [`room.sendBytes()`](/sdk) from the client SDK, skipping the default MsgPack decoding.
