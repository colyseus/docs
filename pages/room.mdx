...
The `client` instance from the backend is responsible for the **transport** layer between the server and the client. Do not confuse it with the [`Client` from the frontend SDK](/sdk/), as they have completely different purposes.
...
<Callout type="info">
    This call will trigger the [`room.onLeave`](/sdk/connection#leaving-a-room) event on the frontend.
</Callout>
