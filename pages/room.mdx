...
You may provide a validation schema using the `validate()` helper with a Zod schema. If validation fails, the client is disconnected with close code `4002` (`WITH_ERROR`) — or, for a request, the request settles as an error.
...
<ScenarioGrid>
    ...
    <ScenarioCard icon={<BroadcastIcon size={18}/>} title="Relay Room" href="/room/relay">
        A lightweight relay that broadcasts client messages to everyone else — no authoritative state.
    </ScenarioCard>
</ScenarioGrid>
