...
Called once, when the room is created by the matchmaker. `options` is the merged values specified on [Server#define()](/server/#define-room-type) with the options provided from the SDK at [`.joinOrCreate()`](/sdk#join-or-create-recommended) or [`.join()`](/sdk#other-join-methods).
...
The `onBeforePatch` lifecycle hook is triggered before state synchronization, at patch rate frequency. (see [`patchRate`](#configuration-properties))
...
Locking removes the room from matchmaking — `join()`/`joinOrCreate()` skip it, and `joinById()` rejects with `"room is locked"`. No new clients can join until it's unlocked.
...
<Callout type="info">
    The client will receive the message in the [`onMessage()`](/sdk#receiving-messages) callback.
</Callout>
...
<Callout type="warning">
    The `metadata` setter can only be used during `onCreate()`. To update metadata after the room is created, use [`setMetadata()`](#matchmaking-properties) or [`setMatchmaking()`](#matchmaking-properties) instead.
</Callout>
...
You operate on `client` instances from [`this.clients`](#clients), [`Room#onJoin()`](#on-join), [`Room#onLeave()`](#on-leave) and [`Room#onMessage()`](#on-messages).
...
The `client.auth` property holds the data returned by the [`onAuth()`](#on-auth) method.
...
<Callout type="info">
    [See how to handle `onMessage` from the frontend SDK](/sdk#receiving-messages)
</Callout>
...
<Callout type="info">
    This will trigger [`room.onLeave`](/sdk#leaving-a-room) event on the frontend.
</Callout>
