...
- **JWT session revocation** — JWTs carry a `tokenVersion` claim; `db.auth.bumpTokenVersion(userId)` invalidates every previously-issued token for that user. This is what powers "Revoke sessions" and "Ban" actions in admin tooling.
