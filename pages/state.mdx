...
- Colyseus optimizes performance and bandwidth by tracking property-level changes. Only the latest mutation of each property is queued and sent to clients during the [patchRate](/room#patchrate) interval.
...
- **Sending changes**: The server encodes only the changed properties and sends them to the client during the [patchRate](/room#patchrate) interval.
