...
description: "How state synchronization works in Colyseus: the server holds authoritative state, mutations are tracked at the property level, and clients receive binary patches."
---
import { Cards, Steps, Tabs } from 'nextra/components'
import { CodeSquareIcon, FileBinaryIcon, EyeIcon, MeterIcon } from '@primer/octicons-react'
import { MovedAnchors } from '../components/moved-anchors'

# State Synchronization

{/* The decorator tsconfig section moved to the decorators page; keep old deep links working.
    The best-practices half of the old combined heading became /state/optimization. */}
<MovedAnchors map={{
    "typescript-config": "/state/schema/decorators#typescript-config",
    "limitations-and-best-practices": "/state#limitations",
}} />
...
<Cards>
    ...
    <Cards.Card icon={<CodeSquareIcon/>} title="State Sync Callbacks" href="/state/callbacks" />
    <Cards.Card icon={<MeterIcon/>} title="Optimizing State" href="/state/optimization" />
</Cards>
...
- Colyseus optimizes performance and bandwidth by tracking property-level changes. Only the latest mutation of each property is queued and sent to clients during the [patchRate](/room#configuration-properties) interval.
- On the frontend, you listen for state changes to keep the user interface in sync.

<Steps>
	### Backend: Define your state structures

    Define your state structures using `schema()` and `t.*` field builders from `@colyseus/schema`, or the classic [`@type()` decorator style](/state/schema/decorators):
...
For what a patch costs in bytes and how to make it smaller, see [Optimizing State](/state/optimization).
...
- **Sending changes**: The server encodes only the changed properties and sends them to the client during the [patchRate](/room#configuration-properties) interval.
...
The `schema()` builder needs no compiler configuration. If you use the [`@type()` decorator style](/state/schema/decorators), two `tsconfig.json` flags are required. See [Decorators → TypeScript Config](/state/schema/decorators#typescript-config).
...
- [Decorators (@type)](/state/schema/decorators) - The classic decorator-based definition style
- [State Sync Callbacks](/state/callbacks) - All methods for listening to state changes
- [State View](/state/view) - Control which parts of state each client can see
- [Optimizing State](/state/optimization) - What a patch costs, and how to make it smaller
