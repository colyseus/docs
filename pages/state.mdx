...
<Cards>
    ...
    <Cards.Card icon={<CodeSquareIcon/>} title="Schema Callbacks" href="/state/callbacks" />
...
<Steps>
    ...
    <Tabs items={["Using .onChange() on an instance", "Using .listen() on a property"]}>
        ...
        callbacks.onAdd("players", (player, sessionId) => {
            ...
            callbacks.listen(player, "y", (x, prevX) => {
                console.log('Player changed:', x, "from", prevX);
            });
        });
        ```
...
- [Schema Callbacks](/state/callbacks) - All methods for listening to state changes
