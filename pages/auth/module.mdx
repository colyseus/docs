---
title: "Auth Module"
description: "The @colyseus/auth module: email/password and anonymous auth, OAuth providers, JWT tokens, email verification, and password reset for Colyseus."
---
...
<Callout type="info">
    **Using [`@colyseus/database`](/database)?** It wires up the user store, registration/login routes, ban gating, and session revocation for you, so you can skip the user-store [Backend API](#backend-api) callbacks below. Email delivery (`onForgotPassword`, `onSendEmailConfirmation`, `onEmailConfirmed`) is still yours to wire. See [Database → Authentication](/database/authentication) for the integration overview.
</Callout>

The `@colyseus/auth` module is highly configurable and allows you to implement your own authentication backend.

The module does not provide a database or email sending capabilities. Instead, it provides callbacks that you can implement to query your database, send emails, and handle OAuth providers.
...
- `SESSION_SECRET` - Used to sign the OAuth state cookie. Recommended for OAuth; falls back to `JWT_SECRET` when unset.
...
<Callout type="warning">
    **`AUTH_SALT` is legacy.** Required only when upgrading an existing deployment from a version prior to 0.18. The legacy `scrypt`-based hasher reads it to verify passwords created before the upgrade. New 0.18 projects don't need it; the default hasher derives its own per-password salt.
</Callout>
...
<Callout type="error" emoji="🚨">
    **Keep Your Secrets Safe!**
    ...
    - Rotating `AUTH_SALT` (legacy upgrades only) will invalidate every pre-upgrade password hash. Users created before the upgrade will need to reset their password.
</Callout>
...
<Tabs items={[<><DevicesIcon/> Client API</>, <><GearIcon/> Backend config</>]}>
    ...
    <Tabs.Tab>
    Each provider requires a different configuration. See the [OAuth providers](#oauth-providers-discord-google-x-etc) section for more information.
...
<Callout type="info">
    **The OAuth Authentication Flow**
    ...
    - A popup window opens `/auth/provider/[PROVIDER-ID]`
    - The user is redirected to the OAuth provider's website
    - The user authenticates with the OAuth provider
    - The user is redirected back to `/auth/provider/[PROVIDER-ID]/callback` (see [OAuth Provider Callback](#oauth-callback))
...
Implement these callbacks when you're bringing your own database. With [`@colyseus/database`](/database), they're wired automatically. See [Database & Persistence → Authentication](/database/authentication).

### Backend URL

The public backend URL. Setting `auth.backend_url` is recommended for security. The value is auto-detected upon the first request if not set.
...
The password provided is already hashed (with `Hash.make`). To change the hashing algorithm, set [`Hash.algorithm`](#on-hash-password).
...
You are responsible for limiting user access to your application until their email is verified.
...
Use this callback to update the user's database record as verified.
...
Password hashing is customized through the `Hash` class. The default algorithm is `scrypt`, with `sha1` available as the only other built-in:

```ts filename="src/config/auth.ts"
import { Hash } from "@colyseus/auth";

Hash.algorithm = "sha1"; // default: "scrypt"
```

<Callout type="warning">
    The `auth.settings.onHashPassword` callback exists on the settings object but is **not invoked by any endpoint in 0.18**, so overriding it has no effect. Use `Hash.algorithm` / `Hash.algorithms` instead.
</Callout>
