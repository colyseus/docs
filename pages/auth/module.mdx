...
<Callout type="info">
    **Using [`@colyseus/database`](/database)?** It wires up the user store, registration/login routes, ban gating, and session revocation for you — you can skip the user-store [Backend API](#backend-api) callbacks below. Email delivery (`onForgotPassword`, `onSendEmailConfirmation`, `onEmailConfirmed`) is still yours to wire. See [Database → Authentication](/database/authentication) for the integration overview.
</Callout>
...
<Callout type="warning">
    The `auth.settings.onHashPassword` callback exists on the settings object but is **not invoked by any endpoint in 0.18** — overriding it has no effect. Use `Hash.algorithm` / `Hash.algorithms` instead.
</Callout>
