...
<Callout type="info">
    **Using [`@colyseus/database`](/database)?** It wires up the user store, registration/login routes, ban gating, and session revocation for you — you can skip the [Backend API](#backend-api) callbacks below. See [Database → Authentication](/database#authentication-built-in-colyseusauth-integration) for the integration overview.
</Callout>
...
Implement these callbacks when you're bringing your own database. With [`@colyseus/database`](/database), they're wired automatically — see [Database & Persistence → Authentication](/database#authentication-built-in-colyseusauth-integration).
