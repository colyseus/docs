...
<Callout type="info">
    **Using [`@colyseus/database`](/database)?** It wires up the user store, registration/login routes, ban gating, and session revocation for you — you can skip the [Backend API](#backend-api) callbacks below. See [Database → Authentication](/database/authentication) for the integration overview.
</Callout>
