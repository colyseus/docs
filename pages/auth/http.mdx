...
<Callout type="info">
    With [`@colyseus/database`](/database) wired, `auth.middleware()` becomes **revocation-aware** automatically: bans and `db.auth.bumpTokenVersion(userId)` invalidate live JWTs at the token layer — no per-route check needed. See [Database → Authentication](/database#authentication-built-in-colyseusauth-integration).
</Callout>
