...
<Callout type="info">
    **Using [`@colyseus/database`](/database)?** `JWT.verify` becomes revocation-aware automatically — banned users and tokens invalidated via `db.auth.bumpTokenVersion()` are rejected at the token layer, before `onAuth` even runs. See [Database → Authentication](/database#authentication-built-in-colyseusauth-integration).
</Callout>
...
When the [`@colyseus/database`](/database#authentication-built-in-colyseusauth-integration) integration is active, `JWT.verify` already enforces session revocation. You only need an explicit `onAuth` check when you want additional, room-level rejection logic — for example, bouncing a banned user before they reach `onJoin`:
