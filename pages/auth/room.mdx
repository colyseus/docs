...
<Callout type="info">
    **Using [`@colyseus/database`](/database)?** A default `onAuth` is installed for you that verifies the JWT and rejects revoked sessions — `db.auth.ban()` and `db.auth.bumpTokenVersion()` both invalidate live tokens at the token layer, so you only need to write a custom `onAuth` for room-specific checks on top. See [Database → Authentication](/database/authentication).
</Callout>
...
When the [`@colyseus/database`](/database/authentication) integration is active, the default `onAuth` verifies the JWT and enforces `tokenVersion`-based revocation — bans and explicit `bumpTokenVersion()` calls both reject stale tokens automatically. Override `onAuth` only for room-specific logic — for example, surfacing the ban reason in the close frame instead of a generic AUTH_FAILED:

```ts {2,7-10} filename="src/rooms/MyRoom.ts"
import { Room, ServerError } from "colyseus";
import { db } from "../app.config";

class MyRoom extends Room {
    async onAuth(client, options, context) {
        // client.auth is already populated by @colyseus/auth's default onAuth
        const { banned, reason } = await db.auth.isBanned(client.auth.id);
        if (banned) {
