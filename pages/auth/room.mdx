...
The client IP address is available in the `context.ip` property, as a single
address. Colyseus reads it from `x-real-ip`, then the first entry of
`x-forwarded-for`, then `x-client-ip`, and falls back to the address of the
connection itself. The property is `undefined` when no source provides one.
