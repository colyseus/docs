...
When the [`@colyseus/database`](/database#authentication-built-in-colyseusauth-integration) integration is active, the default `onAuth` verifies the JWT and enforces `tokenVersion`-based revocation — bans and explicit `bumpTokenVersion()` calls both reject stale tokens automatically. Override `onAuth` only for room-specific logic — for example, surfacing the ban reason in the close frame instead of a generic AUTH_FAILED:
