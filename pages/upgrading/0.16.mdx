...
Make sure `colyseus` and all `@colyseus/*` packages on `package.json` are pointing to `^0.16.0-preview.0`. Having different versions of `colyseus` and `@colyseus/*` packages may lead to unexpected behavior.
...
```json filename="package.json"
    ...
    "@colyseus/auth": "^0.16.0-preview.5",
    ...
    "@colyseus/testing": "^0.16.0-preview.5",
...
If you rely on this method, you can re-implement it on your server-side code:
...
On previous version (`0.15`) it is possible to disable this behaviour, please reach out to the community if you need help on this.
