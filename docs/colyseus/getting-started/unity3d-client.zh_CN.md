...
!!! tip "自 2021 年 5 月 10 日起的变更记录"
    [点击这里查阅更多信息](/migrating/unity-sdk-0.14.5).
...
- 进入 Git URL: `https://github.com/colyseus/colyseus-unity3d.git#upm`
- 点击 "ADD"
...
```
- 如果您的管理器有其他类需要参照您的 `ColyseusClient`, 您可以重写 `InitializeClient` 并在其中建立这些连接.
```csharp
...
- 如果您希望管理器中有多个参考 `ColyseusClient`, 或者如果您想为您的 `ColyseusClient` 提供一个备用 `endpoint` / `ColyseusSettings` 对象, 那么您可以直接调用 `base.InitializeClient()`.
    - 在重写的 `InitializeClient()` 函数中, 您可以将一个终结点传递给任何您创建的其他新的 `ColyseusClient`, 或者您也可以用 `ColyseusSettings` 对象和 `布尔值` 来创建一个新的 `ColyseusClient`, 以表明在建立连接时应使用 websocket 协议而不是 http 协议. 如果您用 `字符串` 终结点创建一个新的 `客户端`, 那么它将在其构造函数中创建一个 `ColyseusSettings` 对象并从终结点推断协议.
```csharp
public override void InitializeClient()
{
...
- 有多种创建或加入房间的方式.
- 您可以调用 `ColyseusClient` 的 `创建` 方法来创建房间, `ColyseusClient` 将自动在服务端上创建房间实例并加入其中:
```csharp
...
### onLeave
!!! tip "更新至 0.14.7 版本"
    为处理 websocket 自定义闭包代码, 委托函数现在从传递 `WebSocketCloseCode` 值改为传递 `int` 闭包代码.
