...
- [Schema 回调 API 改动](#schema-callbacks-api-slightly-changed)
...
现在, 什么时候注册了 `onAdd()` 回调, it is called immediately for already existing items - thus, making the usage of `.triggerAll()` not necessary anymore.
